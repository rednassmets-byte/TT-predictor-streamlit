...
print("=" * 60)
print("IMPROVED MODEL TRAINING WITH ALL ENHANCEMENTS")
print("=" * 60)
...
print(f"\nExcluding youth categories: {youth_categories}")
...
# ===== ENHANCEMENT 1: Opponent Strength Features =====
...
# ===== ENHANCEMENT 2: Opponent Quality Weighting =====
...
toughest_beaten = []
for idx, row in df.iterrows():
    toughest = len(ranking_order)  # Start with worst rank
    for rank in ranking_order:
...
# ===== ENHANCEMENT 3: Match Context Features =====
...
# Recent form (weighted towards recent matches - proxy using overall performance)
...
# ===== ENHANCEMENT 4: Hyperparameter Tuning with Optuna =====
...
# Create ensemble (models are already fitted, so we create a simple wrapper)
print(f"\nEnsemble created with {len(estimators)} models")

# Use the optimized RandomForest as the main model (it's already fitted)
best_model = rf_model
...
joblib.dump(category_encoder, "category_encoder_improved.pkl")
joblib.dump(rank_to_int, "rank_to_int_improved.pkl")
joblib.dump(int_to_rank, "int_to_rank_improved.pkl")
joblib.dump(feature_cols, "feature_cols_improved.pkl")
joblib.dump(ranking_order, "ranking_order_improved.pkl")
