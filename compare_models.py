...
# Load data and prepare features (same as training)
...
feature_cols_v3 = [
    ...
    'improvement_signal', 'decline_signal', 'is_junior', 'junior_volatility',
    'in_de_zone', 'match_reliability', 'reliable_performance', 'is_low_rank', 'is_mid_rank'
]
...
print(f"Comparing on {len(df)} players\n")
...
print("\n" + "=" * 80)
print("ACCURACY BY RANK CHANGE TYPE (KEY METRIC)")
print("=" * 80)
...
juniors = df[df['is_junior'] == 1]
non_juniors = df[df['is_junior'] == 0]

print(f"\nJuniors ({len(juniors)} players):")
print(f"  V2: {juniors['correct_v2'].mean():.2%}")
print(f"  V3: {juniors['correct_v3'].mean():.2%}")
print(f"  Difference: {(juniors['correct_v3'].mean() - juniors['correct_v2'].mean())*100:+.2f}%")

print(f"\nNon-juniors ({len(non_juniors)} players):")
print(f"  V2: {non_juniors['correct_v2'].mean():.2%}")
print(f"  V3: {non_juniors['correct_v3'].mean():.2%}")
print(f"  Difference: {(non_juniors['correct_v3'].mean() - non_juniors['correct_v2'].mean())*100:+.2f}%")

print("\n" + "=" * 80)
print("ACCURACY BY RANK ZONE")
print("=" * 80)

de_zone = df[df['in_de_zone'] == 1]
other_ranks = df[df['in_de_zone'] == 0]

print(f"\nD-E Zone ({len(de_zone)} players):")
print(f"  V2: {de_zone['correct_v2'].mean():.2%}")
print(f"  V3: {de_zone['correct_v3'].mean():.2%}")
print(f"  Difference: {(de_zone['correct_v3'].mean() - de_zone['correct_v2'].mean())*100:+.2f}%")

print(f"\nOther Ranks ({len(other_ranks)} players):")
print(f"  V2: {other_ranks['correct_v2'].mean():.2%}")
print(f"  V3: {other_ranks['correct_v3'].mean():.2%}")
print(f"  Difference: {(other_ranks['correct_v3'].mean() - other_ranks['correct_v2'].mean())*100:+.2f}%")
...
# Weight improvements/declines more heavily since that's what we care about
v2_score = (
    improvements['correct_v2'].mean() * 0.4 +
    declines['correct_v2'].mean() * 0.3 +
    stable['correct_v2'].mean() * 0.3
)

v3_score = (
    improvements['correct_v3'].mean() * 0.4 +
    declines['correct_v3'].mean() * 0.3 +
    stable['correct_v3'].mean() * 0.3
)

print(f"\nWeighted Score (40% improvements, 30% declines, 30% stable):")
print(f"  V2: {v2_score:.2%}")
print(f"  V3: {v3_score:.2%}")
print(f"  Difference: {(v3_score - v2_score)*100:+.2f}%")

if v3_score > v2_score:
    print("\n✓ V3 IS BETTER - Use model_v3_improved.pkl")
else:
    print("\n✗ V2 IS BETTER - Stick with model_v2.pkl")
