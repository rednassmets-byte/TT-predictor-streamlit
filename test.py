...
def add_data_for_club_batch(club: str, season: int, max_workers=10):
    """Add data for all members of a club in a given season to the main CSV file in batch using parallel processing."""
    club_info = get_information_club(club=club, seizoen=season)
    names = extract_names(club_info)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_name = {executor.submit(get_data, club, name, season): name for name in names}
...
if __name__ == "__main__":
    # Read clubs in Antwerpen from CSV
    df_clubs = pd.read_csv("club_data.csv", encoding='utf-8', header=1)
    antwerpen_clubs = df_clubs[df_clubs['Provincie'] == 'Antwerpen']['Clubnr.'].tolist()

    # For each club in Antwerpen, for seasons 15 to 25
    for club in antwerpen_clubs:
        for season in range(15, 26):
            print(f"Processing club {club} for season {season}")
            try:
                add_data_for_club_batch(club=club, season=season, max_workers=10)
            except Exception as e:
                print(f"Error processing club {club} season {season}: {e}")
                continue
