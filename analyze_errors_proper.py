"""
Analyze what the models get most wrong
"""
import pandas as pd
import ast
import joblib
...
# ===== WHAT DOES THE MODEL GET MOST WRONG? =====
...
print("=" * 80)
# Positive = predicted worse than actual (too pessimistic)
# Negative = predicted better than actual (too optimistic)
...
df['actual_change'] = df['current_rank_encoded'] - df['next_rank_encoded']  # Positive = improvement
...
df['activity_level'] = pd.qcut(df['total_matches'], q=4, labels=['Low', 'Medium', 'High', 'Very High'])
...
df['performance_level'] = pd.qcut(df['win_rate'], q=4, labels=['Poor', 'Below Avg', 'Above Avg', 'Excellent'])
print(f"\nAccuracy by performance level:")
for level in ['Poor', 'Below Avg', 'Above Avg', 'Excellent']:
    subset = df[df['performance_level'] == level]
    print(f"  {level}: {subset['correct'].mean():.2%} ({len(subset)} players)")

print("\n" + "=" * 80)
print("KEY INSIGHTS - WHAT THE MODEL GETS MOST WRONG")
print("=" * 80)
